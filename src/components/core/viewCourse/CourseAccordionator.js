...
const CourseAccordionator = ({ setReviewModal }) => {
    ...
    return showCourseMenu && (
        <div className={`w-full sm:w-fit sm:max-w-[20%] sm:min-w-[300px] p-4  sm:min-h-[calc(100vh-4rem)] bg-richblack-800 border-r-2 border-b-2 border-richblack-700`}>
            ...
            <ProgressBar className="w-[80%]" completed={parseInt(((completedVideos?.length) / totalNoOfLectures) * 100)} bgColor={parseInt(((completedVideos?.length) / totalNoOfLectures) * 100) === 100 ? "#03C988" : "#FFD60A"} baseBgColor="#585D69" isLabelVisible={false} height="10px" />
            <p className={`w-[15%] ${parseInt(((completedVideos?.length) / totalNoOfLectures) * 100) === 100 ? "text-[#03C988]" : "text-[#FFD60A]"} text-lg font-bold`}>{parseInt(((completedVideos?.length) / totalNoOfLectures) * 100)}%</p>
            </div>
